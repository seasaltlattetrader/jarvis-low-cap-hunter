...
function score(p){
 ...
 const score=clamp(thesis*.32+momentum*.30+liquidity*.23+structure*.15);
 const verdict=score>=78&&!late?'investigate':score>=58?'watch':'pass';
 const decision=verdict==='investigate'?(late?'Strong setup but extended — wait for reset/reclaim.':'Investigate now — thesis, demand, liquidity and structure align.'):verdict==='watch'?(late?'Interesting but late — wait for reset/reclaim.':'Watch — manual smart-money and holder checks still required.'):'Pass — current evidence does not clear the bar.';
 return{mc,liq,v5,v1,pc5,pc1,buys,sells,momentum,liquidity,structure,thesis,total:score,verdict,late,decision}
}
function coin(p,x){const a=p.address||p.token_address||p.baseToken?.address||p.mint||'';return{id:a,s:'$'+(p.symbol||p.baseToken?.symbol||'?'),name:p.name||p.baseToken?.name||'Unknown',address:a,mc:x.mc,liq:x.liq,age:age(p.recent_listing_time||p.creation_time||p.creationTime||p.created_at||p.pairCreatedAt||0),stage:x.mc<250000?'early':x.mc<1000000?'developing':'expansion',narrative:p.description||p.narrative||'Solana meme candidate',thesis:x.thesis,demand:x.momentum,safety:x.liquidity,setup:x.structure,score:x.total,verdict:x.verdict,why:x.decision,catalyst:`${money(x.v5)} 5m volume • ${money(x.v1)} 1h volume • ${x.pc5.toFixed(1)}% 5m • ${x.pc1.toFixed(1)}% 1h`,decision:x.decision,invalid:'Thesis failure, demand flip, liquidity deterioration, or suspicious wallet/dev behavior.',watch:false,url:p.url||`https://dexscreener.com/solana/${a}`,intel:{verified:false,summary:'Smart-money is intentionally manual in this JARVIS build. Verify wallets yourself before entry.'}}}
