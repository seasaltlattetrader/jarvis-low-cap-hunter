...
function parseCookie(req,name){const raw=req.headers?.cookie||'';const m=raw.match(new RegExp('(?:^|; )'+name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'=([^;]*)'));return m?decodeURIComponent(m[1]):''}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const cooldownUntil=Number(parseCookie(req,'jarvis_birdeye_cooldown'))||0;
  let sm={ok:false,map:new Map(),status:0};
  if(key && Date.now()<cooldownUntil){sources.smartStatus=429;sources.smartCooldownUntil=new Date(cooldownUntil).toISOString()}
  else if(key){sm=await smartMap(key);sources.smart=sm.ok;sources.smartStatus=sm.ok?200:sm.status;if(sm.status===429){const until=Date.now()+10*60*1000;res.setHeader('Set-Cookie',`jarvis_birdeye_cooldown=${encodeURIComponent(String(until))}; Max-Age=600; Path=/; SameSite=Lax`);sources.smartCooldownUntil=new Date(until).toISOString()}}
  // Fast scan intentionally uses ONE smart-money call per cooldown window. Holder/security stay on deep-scan routes.
