...
function score(p){
 ...
 // A call should surface while a low-cap setup is still developing. Strong early momentum can qualify before the absolute score is perfect.
 const earlyEnough=mc<=1500000 && (!p.pairCreatedAt || age(p.pairCreatedAt)!=='—');
 ...
 else if(total>=55){verdict='watch';decision='WAIT — promising, but not enough confluence for a buy setup.'}
 return{mc,liq,v5,v1,pc5,pc1,buys,sells,demand:momentum,liquidity,structure,thesis,total,verdict,late,decision}
}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const out=raw.map(p=>coin(p,score(p))).sort((a,b)=>{const v={investigate:4,watch:3,too_late:2,pass:1};return(v[b.verdict]-v[a.verdict])||(b.score-a.score)}).slice(0,15);
  sources.fresh=true;
