...
function scoreToken(p,sm=null,holder=null,security=null){
 ...
 safety=clamp(safety);
 const hasStory=Boolean(p.symbol||p.name); const thesis=clamp(hasStory?65:45);
 const total=clamp(thesis*.28+demand*.25+smart*.22+safety*.20+setup*.05);
 const verifiedSmart=Boolean(sm||holder);
...
function toCoin(p, x, holder, sm, security){const addr=p.address||p.token_address||p.baseToken?.address||'';return{id:addr,s:'$'+(p.symbol||p.baseToken?.symbol||'?'),name:p.name||p.baseToken?.name||'Unknown',address:addr,mc:x.mc,liq:x.liq,age:ageFrom(p.recent_listing_time||p.pairCreatedAt),stage:x.mc<250000?'early':x.mc<1000000?'developing':'expansion',narrative:p.description||'Low-cap market candidate',thesis:x.thesis,demand:x.demand,smart:x.smart,safety:x.safety,setup:x.setup,score:x.total,verdict:x.verdict,why:x.decision,catalyst:`${money(x.v1)} 1h volume • ${x.pc1.toFixed(1)}% 1h • ${x.buys} buys / ${x.sells} sells`,decision:x.decision,invalid:'Thesis failure, demand flip, liquidity deterioration, or suspicious holder/dev behavior.',watch:false,url:p.url||`https://dexscreener.com/solana/${addr}`,intel:{verified:Boolean(sm||holder),summary:holder?.summary||sm?.summary||'No verified smart-money cohort match for this token.'},security:security||null}}
async function smartMap(key){try{const j=await get(`${BIRD}/smart-money/v1/token/list?interval=1d&trader_style=all&sort_by=net_flow&sort_type=desc&offset=0&limit=20`,hdr(key));const rows=j?.data?.items||j?.data||[];const m=new Map();for(const r of rows){const a=r.address||r.token_address||r.tokenAddress||r.mint;if(a)m.set(a.toLowerCase(),r)}return{ok:true,map:m}}catch{return{ok:false,map:new Map()}}}
async function holderProfile(address,key){try{const j=await get(`${BIRD}/token/v1/holder-profile?token_address=${encodeURIComponent(address)}&interval=1h&ui_amount_mode=scaled&include_zero_balance=true`,hdr(key));const d=j?.data||null;if(!d)throw Error('empty');const tags=d.tags||{};const share=k=>num(tags?.[k]?.percent_of_supply||tags?.[k]?.supply_share);const count=k=>num(tags?.[k]?.holder_count||tags?.[k]?.count);const bad=share('bundler')+share('sniper')+share('insider')+share('dev');const smartSupply=share('smart_trader');const smartNo=count('smart_trader');const top10=num(d.top10_holder?.percent_of_supply||d.top10_holder?.supply_share);return{tags,verified:true,smartSupply,smartNo,badSupply:bad,top10,summary:`Smart traders ${smartNo} (${smartSupply.toFixed(2)}% supply) • suspicious cohorts ${bad.toFixed(2)}% • top-10 ${top10.toFixed(2)}%`}}catch{return null}}
async function security(address,key){try{const j=await get(`${BIRD}/defi/token_security?address=${encodeURIComponent(address)}`,hdr(key));return j?.data||null}catch{return null}}
async function dexFallback(){try{const pr=await get(`${DEX}/token-profiles/latest/v1`);const prof=(Array.isArray(pr)?pr:[]).filter(x=>x.chainId==='solana'&&x.tokenAddress).slice(0,12);const got=await Promise.all(prof.map(async t=>{try{const j=await get(`${DEX}/token-pairs/v1/solana/${t.tokenAddress}`);const ps=Array.isArray(j)?j:[];return ps.sort((a,b)=>num(b.liquidity?.usd)-num(a.liquidity?.usd))[0]||null}catch{return null}}));return got.filter(Boolean)}catch{return[]}}
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');const key=process.env.BIRDEYE_API_KEY||'';const sources={dex:false,birdeye:Boolean(key),smart:false,holder:false,security:false,fresh:false};try{
 let raw=[];
 if(key){
   const q=new URLSearchParams({sort_by:'v24hUSD',sort_type:'desc',min_market_cap:'50000',max_market_cap:'5000000',min_liquidity:'5000',limit:'30',offset:'0',ui_amount_mode:'scaled'});
   try{const j=await get(`${BIRD}/defi/v3/token/list?${q.toString()}`,hdr(key));raw=(j?.data?.items||j?.data||[]);sources.birdeye=true}catch{}
 }
 if(!raw.length){const pairs=await dexFallback();sources.dex=pairs.length>0;raw=pairs.map(p=>({address:p.baseToken?.address,symbol:p.baseToken?.symbol,name:p.baseToken?.name,marketCap:p.marketCap,fdv:p.fdv,liquidity:p.liquidity?.usd,volume_1h_usd:p.volume?.h1,volume_24h_usd:p.volume?.h24,price_change_1h_percent:p.priceChange?.h1,price_change_5m_percent:p.priceChange?.m5,trade_5m_count:num(p.txns?.m5?.buys)+num(p.txns?.m5?.sells),trade_1h_count:num(p.txns?.h1?.buys)+num(p.txns?.h1?.sells),url:p.url,pairCreatedAt:p.pairCreatedAt,baseToken:p.baseToken}))}
 else sources.dex=true;
 raw=raw.filter(p=>{const mc=num(p.marketCap||p.fdv),liq=num(p.liquidity||p.liquidity_usd);return mc>=50000&&mc<=5000000&&liq>=5000}).slice(0,12);
 const sm=key?await smartMap(key):{ok:false,map:new Map()};sources.smart=sm.ok;
 const enriched=await Promise.all(raw.map(async p=>{const addr=p.address||p.token_address||p.baseToken?.address||'';let srow=sm.map.get(addr.toLowerCase())||null;let h=null,sec=null;if(key&&addr){[h,sec]=await Promise.all([holderProfile(addr,key),security(addr,key)])}const x=scoreToken(p,srow,h,sec);if(h?.verified)sources.holder=true;if(sec)sources.security=true;return toCoin(p,x,h,srow,sec)}));
 sources.fresh=true;const coins=enriched.sort((a,b)=>b.score-a.score).slice(0,12);return res.status(200).json({ok:true,generatedAt:new Date().toISOString(),sources,coins});
}catch(e){return res.status(200).json({ok:false,generatedAt:new Date().toISOString(),sources,error:e?.message||'live provider failure',coins:[]})}}
